...
class SmartAIProcessor {
  ...
  async handleCreateQuiz(input, context, siteName, pageSource, contentPrompt) {
    ...
    topic = contentPrompt || pageSource; // Use contentPrompt as topic if available
  ...
  extractCustomizationDetails(input) {
    ...
    const patterns = [
      /customize\s+(?:the\s+)?(\w+)\s+page\s+(?:and\s+)?make\s+it\s+about\s+(.+)/i,
      /customize\s+(?:the\s+)?(\w+)\s+page\s+for\s+(.+)/i,
      /customize\s+(?:the\s+)?(\w+)\s+(?:and\s+)?(?:make\s+it\s+)?(?:about\s+)?(.+)/i,
      /adapt\s+(?:the\s+)?(\w+)\s+page\s+for\s+(.+)/i,
      /modify\s+(?:the\s+)?(\w+)\s+page.*?(?:for|about|to\s+be\s+about)\s+(.+)/i,
      /change\s+(?:the\s+)?(\w+)\s+page.*?to\s+be\s+about\s+(.+)/i
    ];
  ...
  async handleCustomization(input, context) {
    ...
    try {
      ...
      const newTitle = `${sourcePage.title} - ${this.capitalizeWords(customization)}`;
  ...
  async generateQuizFromPageContent(siteName, pageSource, sitesDir, fallbackTopic = null) {
    try {
      // Try to read the page content first
      const pagePath = `${sitesDir}/${siteName}/src/${pageSource.toLowerCase().replace(/\s+/g, '-')}.md`;
      const fs = require('fs').promises;
      let pageContent = '';
      ...
      try {
        ...
        this.log(`Could not read page ${pagePath}, using ${fallbackTopic ? 'fallback topic' : 'page name as topic'}-based quiz`);
        return this.generateQuizFromPrompt(fallbackTopic || pageSource);
      }

      // If we have AI, use it to generate questions
      if (this.hasAPIKey) {
        return await this.generateAIQuizFromContent(pageContent, pageSource);
      }
  ...
  async generateAIQuizFromContent(pageContent, pageSource) {
    const prompt = `Based on this content, create a multiple choice quiz with 3-4 questions. Each question should have 4 options with one correct answer. Format as HTML with <multiple-choice> elements:

Content: ${pageContent.substring(0, 1000)}

Create questions that test understanding of the key concepts. Use this exact format:

<h2>Quiz: ${pageSource}</h2>
<multiple-choice>
  <p>Question 1 text here</p>
  <ul>
    <li>Option A</li>
    <li correct>Correct Option B</li>
    <li>Option C</li>
    <li>Option D</li>
  </ul>
</multiple-choice>
...
<multiple-choice>
  <p>Question 2 text here</p>
  <ul>
    <li>Option A</li>
    <li>Option B</li>
    <li correct>Correct Option C</li>
    <li>Option D</li>
  </ul>
</multiple-choice>`;
  ...
  async generateAIQuizFromTopic(topic) {
    const prompt = `Create a multiple choice quiz about "${topic}" with 3-4 questions. Each question should have 4 options with one correct answer. Format EXACTLY as shown below using proper HAX multiple-choice elements:

<h2>Quiz: ${topic}</h2>
<p>Test your knowledge about ${topic} with this interactive quiz!</p>

<multiple-choice single-option="" randomize="" max-attempts="0" question="What is the largest species of shark?">
  <input type="checkbox" value="Great White Shark">
  <input type="checkbox" value="Whale Shark" correct="correct">
  <input type="checkbox" value="Tiger Shark">
  <input type="checkbox" value="Bull Shark">
</multiple-choice>

<multiple-choice single-option="" randomize="" max-attempts="0" question="How do sharks breathe?">
  <input type="checkbox" value="Through lungs">
  <input type="checkbox" value="Through gills" correct="correct">
  <input type="checkbox" value="Through skin">
  <input type="checkbox" value="They hold their breath">
</multiple-choice>

CRITICAL REQUIREMENTS:
- Put the actual answer text in the value attribute: value="Actual Answer Text"
- Add correct="correct" to the input with the right answer
- DO NOT use placeholder text like "Correct answer" - use real answer text
- Each question must have exactly 4 realistic options
- Make questions educational and relevant to ${topic}
- Include 3-4 questions total`;
