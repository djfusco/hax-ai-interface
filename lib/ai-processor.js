...
class SmartAIProcessor {
  ...
  async handleDeployment(input, context) {
    ...
    // Check if Surge credentials are configured
    const surgeLogin = process.env.SURGE_LOGIN;
    const surgeToken = process.env.SURGE_TOKEN;
    
    if (!surgeLogin || !surgeToken) {
      return {
        explanation: "❌ Surge.sh credentials not configured. Please add your Surge credentials to the .env file:",
        commands: [],
        success: false,
        nextSteps: `Add SURGE_LOGIN and SURGE_TOKEN to your .env file. Get them from https://surge.sh/`,
        examples: [
          "Add to .env file:",
    ...
    return {
      explanation: `🚀 I'll deploy your "${siteName}" site to Surge.sh. This will build the site and make it publicly accessible at ${domain}`,
      commands: commands,
      success: true,
