  ...
  async handleCustomization(input, context) {
    ...
    try {
      ...
      const commands = [
        ...
        `hax site node:add --title "${newTitle}" --content "${customizedContent}" --y`
      ];
  ...
  async generateAISlidedeck(topic, siteName, sitesDir) {
    ...
    try {
      ...
      for (let i = 0; i < slideStructure.slides.length; i++) {
        ...
        const slideCommand = `hax site node:add --title "${sanitizedTitle}" --content "${this.escapeQuotes(slideContent)}" --y`;
